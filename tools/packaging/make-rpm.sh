...
docker build -t "${IMAGE_TAG}" -f ubi-rpm.Dockerfile --build-arg="API_VERSION=${API_VERSION}" --build-arg="VERSION=${ARMONIK_SDK_VERSION_DEFAULT}" --progress=plain "${working_dir}"

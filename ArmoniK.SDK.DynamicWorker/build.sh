...
source "${script_path}"/../tools/common.sh
...
docker build --rm  --no-cache -t "$image_tag" -f "$dockerfile" --build-arg="API_VERSION=${api_version}" --build-arg="WORKER_VERSION=${worker_version}" "$working_dir"

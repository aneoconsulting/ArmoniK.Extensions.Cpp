...
A `Properties` object bundles a `Configuration` (control plane address, mTLS certificates, batching/retry/backpressure knobs) and a default `TaskOptions`.

`Configuration` is loaded from a JSON file (`add_json_configuration`) and/or environment variables (`add_env_configuration`, which take precedence); see the [environment variables reference](4.environment-variables.md) for every key it reads (`GrpcClient__Endpoint`, `GrpcClient__CertPem`/`CertP12`/`KeyPem`/`CaCert`, `GrpcClient__AllowUnsafeConnection`, etc.).

`TaskOptions` is not read from configuration — it's built directly in code via its constructor (application name/version/namespace/service, partition ID, priority, max retries, max duration; see `TaskOptions.h`). For the convention path, the `DynamicLibrary` fields are encoded into it via `SetDynamicLibrary`.

...
| `LibraryPath` | `SetDynamicLibrary` | Path to the `.so`. When `LibraryBlobId` is absent the worker `dlopen`s this path directly (must be a valid path on the **worker** filesystem). When `LibraryBlobId` is present the worker ignores this field and resolves the library from blob storage instead. |
| `LibraryBlobId` | `UploadLibrary` + `SetDynamicLibrary` | Blob ID of the uploaded `.so`; worker downloads it to a temp path and `dlopen`s it at runtime. When set, `LibraryPath` is not used by the worker. |
...
For new code, use `TaskDefinition` and override the three-parameter `HandleResponse`. If you have existing code that overrides the two-parameter `HandleResponse(result_payload, taskId)`, it will still compile and run — the SDK calls the three-parameter version internally and the default implementation delegates to the two-parameter one — but the two-parameter overload is deprecated and will be removed in a future release.
...
#### Option A — library pre-installed on the worker filesystem
...
#### Option B — upload the `.so` as a blob

Use this when the library is not pre-installed on the worker. `UploadLibrary` reads the file from the client filesystem, uploads it to ArmoniK blob storage, and sets `lib.library_blob_id`. The worker then downloads and `dlopen`s it at task execution time; `library_path` is not used by the worker.

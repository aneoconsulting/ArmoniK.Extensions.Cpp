...
namespace SDK_CLIENT_NAMESPACE::Internal{
    ...
    SessionServiceImpl::Submit(const std::vector<TaskRequest> &task_requests, std::shared_ptr<IServiceInvocationHandler> handler, const Common::TaskOptions& task_options) {
    ...
    SessionServiceImpl::SessionServiceImpl(const Common::Properties &properties) : taskOptions(properties.taskOptions), client(armonik::api::grpc::v1::submitter::Submitter::NewStub()){

    }

    std::vector<std::string> SessionServiceImpl::Submit(const std::vector<TaskRequest> &task_requests,

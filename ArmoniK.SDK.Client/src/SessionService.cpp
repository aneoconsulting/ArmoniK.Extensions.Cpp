#include <sstream>
#include <armonik/common/GuuId.h>
...
std::vector<std::string> SDK_CLIENT_NAMESPACE::SessionService::generate_result_ids(size_t num){
    ...
    *results_request.mutable_session_id() = session;
    ...
    std::transform(results_response.results().begin(), results_response.results().end(), std::back_inserter(result_ids), [](auto res){return res.result_id();});
    return result_ids;
}

SDK_CLIENT_NAMESPACE::SessionService::SessionService(const std::shared_ptr<grpc::Channel>& channel,
                                                     const armonik::api::grpc::v1::TaskOptions& taskOptions) : client(armonik::api::grpc::v1::submitter::Submitter::NewStub(channel)),
...
[[maybe_unused]] std::vector<std::string> SDK_CLIENT_NAMESPACE::SessionService::Submit(const std::vector<TaskRequest>& task_requests) {
    ...
    for(int i=0; i<task_requests.size(); i++){
        ...
        *request.mutable_payload() = std::string(task_requests[i].payload.begin(), task_requests[i].payload.end());
        ...
        request.mutable_data_dependencies()->Add(task_requests[i].data_dependencies.begin(), task_requests[i].data_dependencies.end());
    }
    auto list = client.create_tasks_async(session, taskOptions, definitions).get().creation_status_list().creation_statuses();
    std::vector<std::string> task_ids;
    task_ids.reserve(task_requests.size());

    std::transform(list.begin(), list.end(), std::back_inserter(task_ids), [](const armonik::api::grpc::v1::submitter::CreateTaskReply_CreationStatus& status){return status.task_info().task_id();});
    return task_ids;
}
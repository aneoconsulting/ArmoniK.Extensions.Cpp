...
// ---------------------------------------------------------------------------
// Task chaining end-to-end tests
//
// Uses ConventionService (ConventionArithmetic) which parses JSON inputs and
// returns plain numeric strings. BlobDefinition::FromBlobId wires a completed
// task's result blob directly as input to a subsequent task.
// ---------------------------------------------------------------------------

static std::tuple<ArmoniK::Sdk::Common::Properties, armonik::api::common::logger::Logger> init_arithmetic() {
  ArmoniK::Sdk::Common::Configuration config;
  config.add_json_configuration("appsettings.json").add_env_configuration();
...
TEST(SessionService, task_definition_chained_square_then_add) {
  auto p = init_arithmetic();
  auto &properties = std::get<0>(p);
  auto &logger = std::get<1>(p);
  ...
  // Submit tasks A and B in parallel (square 2 and square 3)
  ...
  // opts_add: same as opts_square but with symbol="add"

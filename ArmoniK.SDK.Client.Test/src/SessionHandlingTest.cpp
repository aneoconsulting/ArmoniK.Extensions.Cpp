...
static std::tuple<ArmoniK::Sdk::Common::Properties, armonik::api::common::logger::Logger> init_convention() {
  ...
  // Build the library path the DynamicWorker worker process will use —
  // same location that UseApplication() resolves from.
  std::string base_path = config.get("Worker__ApplicationBasePath");
  if (base_path.empty()) {
    base_path = "/data";
  }
  const std::string version = config.get("WorkerLib__Version");
  const std::string library_path =
      base_path + "/libArmoniK.SDK.Worker.Test.so" + (version.empty() ? "" : "." + version);
